...
Employee.getEmployeeByID = (id,result) => {
    dbConn.query('SELECT * FROM employees WHERE id=?', id, (err,res) => {
        if(err){
            console.log('Error while fetching employees data by Id',err);
            result(null,err);
        }else{
...
Employee.createEmployee = (employeeReqData,result) => {
    dbConn.query('INSERT INTO employees SET ?', employeeReqData , (err,res) => {
        if(err){
            ...
            result(null,res);
        }//(null,{status: false ,message:err})
    })//(null, {status: true,message:'Employee created successfully', insertId: res.Id})
}
